...
exports.linkParentChildReferences = nodes => {
  return nodes.map(node => {
    const type = String(node.internal.type).replace(typePrefix, ''); // look for nodes with child ids (set by Greenhouse)

    if (isNotEmpty(node.child_ids)) {
      node.children = node.child_ids.filter(id => typeof id === "number").map(id => {
  ...
  return nodes.map(node => {
    ...
    if (node.internal.type === 'GreenhouseOffice') {
      ...
      if (isNotEmpty(node.departments)) {
        let officeJobNodeIds = new Array(); // iterate on the list of departments and capture their node ids
        ...
        const officeDepartmentNodeIds = node.departments.map(department => {
          ...
          if (isNotEmpty(department.jobs)) {
            ...
            officeJobNodeIds = officeJobNodeIds.concat(jobNodeIds); // remove the element as it will be rebuilt by Gatsby
        ...
        delete node.departments; // link the office to its departments

        node.departments___NODE = officeDepartmentNodeIds; // link the offices to its jobs
    ...
    if (node.internal.type === 'GreenhouseDepartment') {
      ...
      if (isNotEmpty(node.jobs)) {
        ...
        delete node.jobs; // link the department to its jobs
    ...
    if (node.internal.type === 'GreenhouseJob') {
      ...
      if (isNotEmpty(node.departments)) {
        ...
        delete node.departments; // link the job to its department(s)
      ...
      if (isNotEmpty(node.offices)) {
        ...
        delete node.offices; // link the job to its office(s)
